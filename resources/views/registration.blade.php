...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <form action="{{route('register')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST') 
                <h1 class="text-white text-5xl py-2 pb-10">Registration
                </h1>
                <div class="flex flex-col">
                    <label for="name" class="text-white">name</label>
                    <input  class="border-2 border-black rounded-lg" type="text" name="name" id="name">
                </div>
                <div class="flex flex-col">
                ...
                <button type="submit" class="w-full bg-blue-500 text-white px-4 py-2 mt-4 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Register</button>
            </form>
    </div>
    </body>
</html>
